...
var storage = multer.diskStorage({
  ...
  filename: function (req, file, callback) {
    callback(
      null,
      uuidname + '-' + file.originalname + path.extname(file.originalname)
    );
  },
});
...
module.exports = function (app, passport) {
  ...
  app.get('/', isLoggedIn, function (req, res) {
    ...
    connection.query(query, [req.user.login_id], function (err, rows) {
      ...
      if (req.user.role != 'Patient') {
        ...
        var query2 =
          'select patient_1.*,login.username from patient_1 inner join login on patient_1.login_id=login.login_id where patient_1.login_id= ?';
  ...
  app.post('/addDoctor', function (req, res) {
    ...
    var check =
      'select doc_id, doc_name,contact,specialization from doctor_1 where doc_name=? and contact=?';
  ...
  app.get('/invoiceHistory', isLoggedIn, function (req, res) {
    ...
    connection.query(query, function (err, rows) {
      res.render('./Employees/invoice_history.ejs', {
        user: req.user,
        rows: rows,
      });
    });
  ...
  app.get('/AllRequests', isLoggedIn, function (req, res) {
    app.use(express.static('public'));
    var query =
      "select Status,message_id,imgsrc,message,\
    ...
    connection.query(query, function (err, rows) {
      ...
      console.log(rows);
      res.render('./Employees/patientMessages.ejs', {
        user: req.user,
        rows: rows,
      });
    });
  ...
  app.get('/messageNo:message_id', isLoggedIn, function (req, res) {
    ...
    connection.query(query, [messageId], function (err, rows) {
      ...
      res.render('./Employees/messageDetails.ejs', {
        user: req.user,
        rows: rows,
      });
      console.log(rows);
  ...
  INNER JOIN medicine ON bill_2.med_id=medicine.med_id WHERE bill_1.bill_no = ?";
    ...
    connection.query(query, [billId], function (err, rows) {
      ...
      res.render('./Patients/patbillDetails.ejs', {
        user: req.user,
        rows: rows,
      });
    });
  ...
  app.get('/PatRequest', isLoggedIn, function (req, res) {
    if (req.user.role === 'Patient') {
      var query =
        'select message,date,Status from message inner join patient_1 on message.pat_id=patient_1.pat_id \
  ...
  app.get('/pathome', isLoggedIn, function (req, res) {
    var query =
      'select * from patient_1 inner join login on patient_1.login_id=login.login_id where patient_1.pat_name=? and login.password=?';
    console.log(req.user);
    connection.query(
      query,
    ...
    if (req.user.role === 'Patient') {
      ...
      var upload = multer({
        ...
        fileFilter: function (req, file, callback) {
          ...
          console.log(file);
          var fileloc =
            process.env.IMGDIR + '/' + uuidname + '-' + file.originalname + ext;
          ...
          callback(null, true);
          var query =
            'select pat_id from patient_1 where patient_1.login_id = ?';
          connection.query(query, [temp], function (err, rows) {
            // console.log(req);
            // console.log(req.body);
            ...
            var query2 =
              'insert into message (pat_id,message,imgsrc) values(?,?,?)';
            connection.query(
              query2,
              [pid, message, fileloc],
              function (err, rows0) {
  ...
  app.get('/patCreateBill', isLoggedIn, function (req, res) {
    var today = new Date();
    console.log('patient create bill');
    var dd = String(today.getDate()).padStart(2, '0');
    var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
    var yyyy = today.getFullYear();
    ...
    connection.query(query1, function (err, rows) {
      ...
      let getUserDetails =
        'SELECT patient_1.pat_id, patient_1.pat_name, patient_1.age, doctor_1.doc_name FROM \
			((patient_1 INNER JOIN patient_2 ON patient_1.pat_id = patient_2.pat_id) \
			INNER JOIN doctor_1 ON patient_2.doc_id=doctor_1.doc_id) WHERE patient_1.login_id = ?';
      console.log(req.user);
      connection.query(
        getUserDetails,
        [req.user.login_id],
        function (err, rows2) {
          ...
          connection.query(
            'SELECT MAX(bill_no) AS bill_no FROM bill_1',
            function (err, bills) {
              res.render('./Patients/patCreateBill.ejs', {
                user: req.user,
                rows: rows,
                rows2: rows2,
                today: today,
